...
// Расширенный интерфейс — добавлен Exec для Register
type DB interface {
...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.QueryRow(
		r.Context(),
		`SELECT id, username, password_hash, tenant_id, type, status
		 FROM users WHERE username = $1`,
		req.Username,
	).Scan(&u.ID, &u.Username, &u.PasswordHash, &u.TenantID, &u.UserType, &u.Status)
